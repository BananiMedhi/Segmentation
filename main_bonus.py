...
def superpixels_histograms_neighbors(img):
    # SLIC
    segments = slic(img, n_segments=520, compactness=23) #18, 29, 33
...
def find_superpixels_under_marking(marking, superpixels):
    ...
    segments_mask = np.all(marking==(0,0,255), axis=-1)
    fg_segments = np.unique(superpixels[segments_mask])
    segments_mask = np.all(marking==(255,0,0), axis=-1)
    bg_segments = np.unique(superpixels[segments_mask])
...
def draw_circle(event,x,y,flags,param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if drawing == True:
            cv2.line(param[1],(ix,iy),(x,y),tuple(color),5)
            cv2.line(param[0],(ix,iy),(x,y),tuple(color),5)
            ix,iy = x,y

    elif event == cv2.EVENT_LBUTTONUP:
        drawing = False

def generate_marking(ast):
    ast = cv2.imread('astronaut.png')
    img = np.ones(ast.shape, np.uint8)
    img = img*255
    cv2.namedWindow('image')
    cv2.setMouseCallback('image',draw_circle, [ast, img])
...
if __name__ == '__main__':
    ...
    img_marking = generate_marking(img)
    ...
    cv2.imshow('img', img)
    cv2.imshow("mask", np.array(segmask).astype('uint8'))
    #cv2.imshow("final", np.array(final).astype('uint8'))
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    ...
    cv2.imwrite(output_name, np.array(segmask).astype('uint8'))
    print RMSD(cv2.imread(output_name),cv2.imread("example_output.png"))
